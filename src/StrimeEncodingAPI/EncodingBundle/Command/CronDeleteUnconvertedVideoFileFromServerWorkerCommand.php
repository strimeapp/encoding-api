...
class CronDeleteUnconvertedVideoFileFromServerWorkerCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('cron:worker:delete-unconverted-video-file-from-server')
            ->setDescription('Deletes the video files that have not been used for more than a certain time')
        ;
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        // Set the video file lifetime
        $video_file_lifetime = (int)$this->getContainer()->getParameter('video_file_unconverted_lifetime');
        $output->writeln( "[".date("Y-m-d H:i:s")."] Set the video files lifetime to ".$video_file_lifetime." seconds." );
        ...
        if(count($files) > 0) {
            ...
            foreach ($files as $file) {
                ...
                if($difference_with_current_time > $video_file_lifetime) {
